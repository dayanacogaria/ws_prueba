...
// Model for Usuario entity
    models.Usuario = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/ws/webresources/entity.usuario/",
        idAttribute: 'indentificacion',
        defaults: {
            apellido: "",
            nombre: ""
        },
        toViewJson: function () {
            ...
            result.displayName = this.get('indentificacion');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/ws/webresources/entity.usuario/';
            }
    ...
    // Collection class for Usuario entities
    models.UsuarioCollection = Backbone.Collection.extend({
        model: models.Usuario,
        url: "http://localhost:8080/ws/webresources/entity.usuario/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UsuarioCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-usuario-list-itemis template identifier for item
                        templateName: '#tpl-usuario-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.usuario = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.usuario,
                    // tpl-usuario-details is template identifier for chosen model element
                    templateName: '#tpl-usuario-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Usuario();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-usuario-details is a template identifier for chosen model element
                templateName: '#tpl-usuario-details',
